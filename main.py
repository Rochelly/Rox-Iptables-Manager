...
def reloadRules(screen):
    ...
    if len(modifiedFiles) == 0:
        screenTemplate(screen)
        screen.addstr(19 ,4, "Não Existe arquivos modificados", curses.color_pair(2))
        return
        ...
        for file in modifiedFiles:
            ...
            screen.addstr(printline ,4, msg, curses.color_pair(4))
    ...
    if listErros:
        screenTemplate(screen)
        for alert in (listErros):
            screen.addstr(printline ,10, alert, curses.color_pair(3))
            printline=printline+1
    else:
        printline=17
        screenTemplate(screen)
        screen.addstr(printline ,20, "Nenhum erro encontrado, regras recarregadas com sucesso!", curses.color_pair(1))
