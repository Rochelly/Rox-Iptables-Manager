            ...
            for line in file:
                ...
                if statu_are_line < self.max_row -2:
                    ...
                    if len(parts) >= 3:
                        ...
                        if level == 'ERROR':
                            self.screen.addstr(statu_are_line, 10, message, curses.color_pair(3))
                        if level == 'INFO':
                            self.screen.addstr(statu_are_line, 10, message, curses.color_pair(1))
                        if level == 'DEBUG':
                            self.screen.addstr(statu_are_line, 10, message, curses.color_pair(5))
                else:
                    msg=f"O tamanho maximo do terminal atigindo ({self.max_row} X {self.max_col})! - Amplie a janela para ver mais "
                    self.screen.addstr(statu_are_line,2, msg, curses.color_pair(4))
                    break         
           # file.truncate(0)
